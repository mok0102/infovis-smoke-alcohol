...
Promise.all([
  ...
  .then(function(data){
    var or_female_male = data[0].filter(d => !isNaN(d.or_male) && !isNaN(d.or_female));
    ...
    // const bubble_trans = d3.select("#bubble_trans")
    //     .on("click", function(d){
    //         if (isPlaying) {
    ...
    //             isPlaying = true;
    //             interval = setInterval(updateBubble(or_female_male), animationDuration + animationDelay);
    //           }
    //     })

    const parellel = d3.select("#linear")
      .on("click", function(d){
        console.log("line clicked!")
        drawParellel(or);
      })

    // rigid는 alcohol, drug중에 선택하도록 그리기
    // 일단 지금은 or로
    const rigid = d3.select("#parellel")
      .on("click", function(d){
        drawRigid(or);
      })
...
function updateBubble(data){
    ...
    // Animate the transition of bubble positions

    var filtered = data.filter(d=> d.Year===currentYear)


    bubbles.data(filtered, d=>d.Entity);
    ...
    bubbles
        ...
        .attr("r", function(d){
            if (d.Year==currentYear) return sizeScale(d.population);
            else return;
        })
...
function drawRigid(data){
    ...
    const categories = d3.group(data, d=>d.Entity);
...
function drawParellel(data) {
    ...
    svg.append("text")
        ...
        .style("text-anchor", "middle")
        .text("Alcohol or Drug Use Disorders (Male&Female)");
